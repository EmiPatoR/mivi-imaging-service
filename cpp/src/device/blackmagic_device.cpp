...
void BlackmagicDevice::queryCapabilities() {
    ...
    capabilities_.deviceInfo["driver_version"] = "12.0"; // Would be queried in real implementation
}
...
void BlackmagicDevice::updatePerformanceMetrics(IDeckLinkVideoInputFrame* videoFrame) {
    ...
    // Update FPS history
    {
        ...
        if (fpsHistory_.size() > 60) { // Keep 1 second of history at 60 fps
...
} // namespace medical            pixelFormat = bmdFormat8BitBGRA;
...
UltrasoundDevice::Capabilities BlackmagicDevice::getCapabilities() const {
...
UltrasoundDevice::Status BlackmagicDevice::setExternalMemory(void* externalMemory, size_t size) {
...
UltrasoundDevice::Status BlackmagicDevice::setDirectOutputToSharedMemory(const std::string& sharedMemoryName) {
...
std::shared_ptr<Frame> BlackmagicDevice::convertFrameExternalMemory(IDeckLinkVideoInputFrame *videoFrame,
                                                                   IDeckLink#include "device/blackmagic_device.h"
