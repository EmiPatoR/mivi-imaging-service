...
namespace medical::imaging {
    /**
     * @class WebcamImagingService
     * @brief Alternative imaging service that uses webcams instead of BlackmagicDevice
     *
     * This class provides the same interface as ImagingService but uses webcams for
     * frame acquisition when Blackmagic devices are not available.
     */
    class WebcamImagingService {
    public:
        /**
        ...
        struct Config {
            ...
            WebcamDevice::Config deviceConfig;      // Device-specific configuration

            // Performance settings
            bool enableDirectMemoryAccess; // Not used for webcams but kept for API compatibility
            ...
            Config() : deviceId(""),
                       enableDirectMemoryAccess(true), // Not used but kept for compatibility
    ...
    class WebcamImagingServiceManager {
    public:
        static WebcamImagingServiceManager &getInstance();

        std::shared_ptr<WebcamImagingService> createService(const std::string &serviceName = "default");

        std::shared_ptr<WebcamImagingService> getService(const std::string &serviceName = "default");
    ...
    private:
        WebcamImagingServiceManager() = default;

        ~WebcamImagingServiceManager();

        std::mutex mutex_;
        std::unordered_map<std::string, std::shared_ptr<WebcamImagingService>> services_;
    };
} // namespace medical::imaging