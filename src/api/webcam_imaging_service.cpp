...
namespace medical::imaging {
    ...
    WebcamImagingService::WebcamImagingService()
    ...
    WebcamImagingService::Status WebcamImagingService::initialize(const Config &config) {
    ...
    WebcamImagingService::Status WebcamImagingService::setupDevice() {
        // Get the device
        auto &deviceManager = WebcamDeviceManager::getInstance();
        ...
        // Convert the config type
        WebcamDevice::Config deviceConfig;
        deviceConfig.width = config_.deviceConfig.width;
        deviceConfig.height = config_.deviceConfig.height;
        deviceConfig.frameRate = config_.deviceConfig.frameRate;
        deviceConfig.pixelFormat = config_.deviceConfig.pixelFormat;
        deviceConfig.enableAudio = config_.deviceConfig.enableAudio;
        
        // Initialize the device
        if (device_->initialize(deviceConfig) != WebcamDevice::Status::OK) {
            return Status::DEVICE_ERROR;
        }
    ...
    // Static methods using the WebcamDeviceManager directly
    std::vector<std::string> WebcamImagingService::getAvailableDevices() {
        auto &deviceManager = WebcamDeviceManager::getInstance();
        return deviceManager.getAvailableDeviceIds();
    }

    WebcamImagingService::Status WebcamImagingService::setupSharedMemory() {
        try {
    ...
    WebcamImagingService::Status WebcamImagingService::start() {
        if (!isInitialized_) {
        ...
        if (config_.enablePerformanceMonitoring) {
            stopRequested_ = false;
            performanceThread_ = std::thread(&WebcamImagingService::performanceMonitorThread, this);
        ...
        auto status = device_->startCapture(
            [this](const std::shared_ptr<Frame> &frame) {
                handleNewFrame(frame);
            });

        if (status != WebcamDevice::Status::OK) {
    ...
    WebcamImagingService::Status WebcamImagingService::stop() {
        if (!isRunning_) {
    ...
    bool WebcamImagingService::isRunning() const {
        return isRunning_;
    }

    WebcamImagingService::Config WebcamImagingService::getConfig() const {
        return config_;
    }

    WebcamImagingService::Status WebcamImagingService::setConfig(const Config &config) {
    ...
    WebcamImagingService::Status WebcamImagingService::setFrameCallback(
        const std::function<void(std::shared_ptr<Frame>)> &callback) {
        frameCallback_ = callback;
        return Status::OK;
    }

    WebcamImagingService::PerformanceMetrics WebcamImagingService::getPerformanceMetrics() const {
        std::lock_guard<std::mutex> lock(metricsMutex_);
        return metrics_;
    }

    void WebcamImagingService::resetPerformanceMetrics() {
        std::lock_guard<std::mutex> lock(metricsMutex_);
    ...
    std::map<std::string, std::string> WebcamImagingService::getStatistics() const {
    ...
    std::shared_ptr<SharedMemory> WebcamImagingService::getSharedMemory() const {
        return sharedMemory_;
    }

    bool WebcamImagingService::dumpDiagnostics(const std::string &filePath) const {
        try {
            ...
            outFile << "Webcam Diagnostic Report: " << std::ctime(&nowTime) << std::endl;
            ...
            if (device_) {
                ...
                outFile << "Current Resolution: " << deviceConfig.width << "x" << deviceConfig.height << std::endl;
                outFile << "Frame Rate: " << deviceConfig.frameRate << std::endl;
                outFile << "Pixel Format: " << deviceConfig.pixelFormat << std::endl;
    ...
    void WebcamImagingService::handleNewFrame(const std::shared_ptr<Frame> &frame) {
        if (!frame) {
            return;
        }
    ...
    void WebcamImagingService::performanceMonitorThread() {
    ...
    void WebcamImagingService::updatePerformanceMetrics() {
        std::lock_guard<std::mutex> lock(metricsMutex_);
    ...
    bool WebcamImagingService::setThreadPriority(std::thread &thread, bool isRealtime, int priority) {
    ...
    bool WebcamImagingService::setThreadAffinity(std::thread &thread, int cpuCore) {
    ...
    // WebcamImagingServiceManager implementation
    WebcamImagingServiceManager &WebcamImagingServiceManager::getInstance() {
        static WebcamImagingServiceManager instance;
        return instance;
    }

    std::shared_ptr<WebcamImagingService> WebcamImagingServiceManager::createService(const std::string &serviceName) {
        ...
        // Create new service
        auto service = std::make_shared<WebcamImagingService>();
        services_[serviceName] = service;
        return service;
    }

    std::shared_ptr<WebcamImagingService> WebcamImagingServiceManager::getService(const std::string &serviceName) {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    bool WebcamImagingServiceManager::destroyService(const std::string &serviceName) {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    void WebcamImagingServiceManager::destroyAll() {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    WebcamImagingServiceManager::~WebcamImagingServiceManager() {
        destroyAll();
    }
} // namespace medical::imaging