...
namespace medical::imaging {
    ...
    struct Frame::Impl {
        ...
        bool initializeGPUMemory(int w, int h, int bpp, const std::string &fmt) {
            width = w;
            height = h;
            bytesPerPixel = bpp;
            format = fmt;
            ...
            // Use CudaHelper to allocate GPU memory
            auto &cudaHelper = CudaHelper::getInstance();
            cudaError_t status = cudaHelper.allocateMemory(&gpuPtr, dataSize);

            if (status != cudaSuccess) {
                return false;
            }

            // Create CPU-accessible pointer to the GPU memory if needed
            data = nullptr; // Will be mapped on-demand when lock() is called
    ...
    std::shared_ptr<Frame> Frame::create(
        ...
        switch (bufferType) {
            ...
            case BufferType::GPU_MEMORY:
                if (!frame->impl_->initializeGPUMemory(width, height, bytesPerPixel, format)) {
                    return nullptr;
                }
                return nullptr;
    ...
    bool Frame::lock(bool readOnly) {
        if (impl_->bufferType == BufferType::GPU_MEMORY) {
            if (impl_->isLocked) {
                return true; // Already locked
            }

            // If we don't have a CPU-accessible pointer, create one
            if (!impl_->data && impl_->gpuPtr) {
                auto &cudaHelper = CudaHelper::getInstance();

                if (readOnly) {
                    // For read-only access, we need to copy data from GPU to a temporary CPU buffer
                    impl_->data = malloc(impl_->dataSize);
                    if (!impl_->data) {
                        return false;
                    }

                    cudaError_t status = cudaHelper.copyDeviceToHost(
                        impl_->data, impl_->gpuPtr, impl_->dataSize);
                    ...
                    // For read-write access, map GPU memory to CPU
                    // This depends on CUDA capability and might not always be possible
                    impl_->data = cudaHelper.mapGpuMemoryToCpu(impl_->gpuPtr, impl_->dataSize);
                    if (!impl_->data) {
                        // If mapping fails, fall back to copy
                        impl_->data = malloc(impl_->dataSize);
                        if (!impl_->data) {
                            return false;
                        }
            ...
            impl_->isLocked = true;
            impl_->isLockedForWriting = !readOnly;
            return true;
        } else {
            // Existing CPU memory handling
            return impl_->lockForCPU(!readOnly);
        }
    }

    void Frame::unlock() {
        if (impl_->bufferType == BufferType::GPU_MEMORY && impl_->isLocked) {
            // If we're unlocking a write lock, copy data back to GPU
            if (impl_->isLockedForWriting && impl_->data && impl_->gpuPtr) {
                auto &cudaHelper = CudaHelper::getInstance();
                cudaHelper.copyHostToDevice(impl_->gpuPtr, impl_->data, impl_->dataSize);
            }

            // Clean up temporary buffer if needed
            if (impl_->data) {
                // If this was a mapped pointer, unmap it
                if (impl_->isLockedForWriting) {
                    auto &cudaHelper = CudaHelper::getInstance();
                    cudaHelper.unmapGpuMemory(impl_->data);
                } else {
                    // Otherwise free the temporary copy
                    free(impl_->data);
                }
                impl_->data = nullptr;
            }
            ...
            impl_->isLockedForWriting = false;
        } else {
            // Existing unlock functionality
            impl_->unlock();
        }
    }
